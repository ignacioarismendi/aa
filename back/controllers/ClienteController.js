...
const obtener_direccion_todos_cliente = async function(req,res){
    if(req.user){
        var id = req.params['id'];
        let direcciones = await Direccion.find({cliente:id}).populate('cliente').sort({createdAct:-1});
...
const obtener_ordenes_cliente = async function(req,res){
    if(req.user){
        ...
        let reg = await Venta.find({cliente:id}).sort({createdAct:-1});
        if(reg.length >=1){
            res.status(200).send({data:reg});
...
const obtener_review_producto_cliente = async function(req,res){
    ...
    let reg = await Review.find({producto:id}).sort({createdAct:-1});
    res.status(200).send({data:reg});
}
