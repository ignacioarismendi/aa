...
const listar_descuentos_admin = async function(req, res) {
    if (req.user) {
        if (req.user.rol == 'admin') {
            ...
            let data = await Descuento.find({ titulo: new RegExp(filtro, 'i') }).sort({createdAct:-1});
    ...
    if (req.user) {
        if (req.user.rol == 'admin') {
            ...
            if (req.files) {
                ...
                fs.stat('./uploads/descuentos/'+reg.portada, function(err){
                    if(!err){
                        fs.unlink('./uploads/descuentos/'+reg.portada,(err)=>{
                            if(err) throw err;
                        });
                    }
                })
...
const obtener_descuento_activo = async function(req,res){
    let descuentos = await Descuento.find().sort({createdAct:-1});
    var arr_descuentos = [];
